...
int main(void)
{
	...
	char my_string[64], my_string1[64];
	int transferred = 0;
	int received = 0;
	int length = 0;
	//int nbytes = 40;
	...
	while(1)
	{
		e = libusb_bulk_transfer(handle, BULK_EP_IN, my_string1, 64, &received, 1000000);  //64: Max Packet Length
		if(e == 0)
		{
			int i;
			printf("\nReceived: ");
			...
			e = libusb_bulk_transfer(handle, BULK_EP_OUT, my_string, 5, &transferred, 1000);  //64: Max Packet Length
			if(e == 0)
			{
				printf("\nSend: ");
